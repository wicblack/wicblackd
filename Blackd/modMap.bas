...
Public Const highestTibiaVersionLong As Long = 1001 ' highest known Tibia version (long)
Public Const TibiaVersionDefaultString As String = "10.01" ' highest known Tibia version (string)
Public Const TibiaVersionForceString As String = "10.01" ' set this version by default (string)
Public Const ProxyVersion = "29.2" ' Proxy version ' string version
Public Const myNumericVersion = 29200 ' numeric version
...
' problems872 [03/05/2011 00:47:56 using version 20.0 , with config.ini v872 ] FATAL ERROR [False ] DEBUG CHAIN [  6E SP: ---- packet 6E DEBUG: 1,2,3,4,5,6, ] CLIENT [ 1 ] (Number:9 Description:Subscript out of range Source: Notepad) in packet : ( hex ) 28 00
   ...
   ' debug872= 6E 02 CA 1F 00 1B 00 74 68 65 20 72 65 6D 61 69 6E 73 20 6F 66 20 61 6E 20 65 6C 65 6D 65 6E 74 61 6C 0E 00 01 D7 0B 25  Last good position=2 x=32398;y=32726;z=3
    ...
    Case &H93
      ' blood effects at creature id. New since Tibia 9.9
...
'frmMain.txtPackets.Text = frmMain.txtPackets.Text & vbCrLf & ">>" & frmMain.showAsStr3(packet, True, pos, pos + 37)
...
'        End If
      ...
      Case Else
         debugChain = debugChain & " at subtype -> [" & GoodHex(subType) & "] "
        pos = pos + 10000
      End Select
...
' 10 00 43 61 68 68 7A 69 6E 68 61 20 6F 66 20 55 6E 69
    ...
    Case &HB4
      ...
      '#wiceps
      If (tempb1 <> &H1) And (tempb1 <> &H4) And (tempb1 <> &H14) And (tempb1 <> &H17) Then
        If ((TrainerOptions(idConnection).misc_dance_14min = 1) And _
            (CheatsPaused(idConnection) = False)) Then
        LogOnFile "servermessages.txt", "[ " & CStr(Format(Date, "dd/mm/yyyy")) & " " & CStr(Format(Time, "hh:mm:ss")) & " ] " & mobName
        
        If (StrComp(mobName, "UWAGA! za 5 minut nastapi restart serwera, restart potrwa 5-10min", 1) <> 0) And _
           (StrComp(mobName, "Serwer zostanie wylaczony za 5 minut", 1) <> 0) And _
           (StrComp(mobName, "Serwer zostanie wylaczony za 4 minuty", 1) <> 0) And _
           (StrComp(mobName, "Serwer zostanie wylaczony za 3 minuty", 1) <> 0) And _
           (StrComp(mobName, "Serwer zostanie wylaczony za 2 minuty", 1) <> 0) And _
           (StrComp(mobName, "Serwer zostanie wylaczony za 1 minute", 1) <> 0) And _
           (StrComp(mobName, "UWAGA! CLEAN za 2 minuty!", 1) <> 0) And _
           (StrComp(mobName, "UWAGA! za 1 minute bedzie czyszczenie smieci LEZACYCH na ziemi/depo, prosze podniesc swoje przedmioty i schowac do plecaka! (nie dotyczy domkow)", 1) <> 0) And _
           (StrComp(mobName, "Clean za 5", 1) <> 0) And _
           (StrComp(mobName, "Clean za 4", 1) <> 0) And _
           (StrComp(mobName, "Clean za 3", 1) <> 0) And _
           (StrComp(mobName, "Clean za 2", 1) <> 0) And _
           (StrComp(mobName, "Clean za 1", 1) <> 0) And _
           (StrComp(mobName, "Awansowales w walce na odleglosc.", 1) <> 0) And _
           (StrComp(mobName, "Awansowales w walce piesciami.", 1) <> 0) And _
           (StrComp(mobName, "Awansowales w walce maczuga.", 1) <> 0) And _
           (StrComp(mobName, "Awansowales w walce mieczem.", 1) <> 0) And _
           (StrComp(mobName, "Awansowales w obronie.", 1) <> 0) And _
           (StrComp(mobName, "", 1) <> 0) And _
           (StrComp(mobName, "UWAGA: Zbyt dlugo nic nie robiles. Zostaniesz wyrzucony za 2 minuty.", 1) <> 0) And _
           (StrComp(mobName, "UWAGA: Zbyt dlugo nic nie robiles. Jezeli nie chcesz zostac wyrzucony pociagnij za dzwignie!", 1) <> 0) And _
           (StrComp(mobName, "OK! mozesz afkowac przez kolejne 20 minut", 1) <> 0) And _
           (StrComp(mobName, "Ktos jest po drugiej stronie drzwi!", 1) <> 0) And _
           (InStr(mobName, "Clean zakonczony pomyslnie. Skasowanych itemow: ") = 0) And _
           (InStr(mobName, "Awansowales z ") = 0 And InStr(mobName, "poziomu na") = 0) Then
 
                
             If (StrComp(mobName, "Wykonales zadanie! Udaj sie do NPC Popi i powiedz mu to!", 1) = 0) Then
                    tmpStr = "exiva __task = 1"
                    AddSchedule idConnection, tmpStr, 100
             ElseIf (tempb1 = &H16) And (Left(mobName, 12) = "Methods Men:" Or Left(mobName, 7) = "Etaril:" Or Left(mobName, 9) = "Mohammed:" Or Left(mobName, 7) = "Psujek:") Then
              'nothing
             
             Else
                    GMname(idConnection) = "Wiceps"
                    DangerGMname(idConnection) = "Wiceps"
                    aRes = SendSystemMessageToClient(idConnection, "GM detected ( " & GMname(idConnection) & " ).")
                    tmpStr = "exiva __gm = 1"
                    AddSchedule idConnection, tmpStr, 100
                    tmpStr = "exiva testsound"
                    AddSchedule idConnection, tmpStr, 100
                    tmpStr = "exiva pause"
                    AddSchedule idConnection, tmpStr, 3000
                    tmpStr = "exiva testsound"
                    AddSchedule idConnection, tmpStr, 100
             End If
          End If
      ...
      If sentFirstPacket(idConnection) = True And sentWelcome(idConnection) = False Then
        ...
        Else
          If LimitedLeader = "-" Then
            aRes = GiveGMmessage(idConnection, "Welcome to Blackd Proxy v" & ProxyVersion & " modified by Wiceps ! Official site: www.blackdtools.com", "Blackd")
            DoEvents
          Else
