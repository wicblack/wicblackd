...
Public Function GetCharListPositionPre(idConnection As Integer, ByRef selectedcharacter As String) As Integer
  ...
  GetCharListPositionPre = CInt(theindex)
  Exit Function
returnTheResult:
...
Public Sub setCharListPosition2(idConnection As Integer)
  ...
  Memory_WriteByte adrSelectedCharIndex, &HFF, ProcessID(idConnection)
End Sub
...
Public Sub UpdateExpVars(idConnection As Integer)
  ...
  var_playeds(idConnection) = Thour * 3600 + Tmin * 60 + Tsec
...
While pos <= lastp
  ...
  If (cc = "$") Then
    ...
    Case "playeds"
      theTranslation = var_playeds(idConnection)
    ...
    Case Else ' user variables
      ...
      ElseIf Len(varn) > 4 Then
        ...
        '#wiceps
        ElseIf (Left$(varn, 16) = "monsteronscreen:") Then 'special variable
          theTranslation = CStr(CountOnFloorWiceps(idConnection, Right$(varn, (Len(varn) - 16))))
        '#wiceps
        ElseIf (Left$(varn, 17) = "monsteronscreen2:") Then 'special variable
          theTranslation = CStr(CountOnFloorWiceps2(idConnection, Mid$(varn, 18, (Len(varn) - 19)), Right$(varn, 1)))
        '#wiceps
...
goterr:
  ...
  Exit Function
End Function
Public Function CountOnFloorWiceps(idConnection As Integer, monsterName As String) As Long
        ...
        For s = 1 To 10
          ...
          If tileID = 97 Then
           nameofgivenID = GetNameFromID(idConnection, Matrix(y, X, z, idConnection).s(s).dblID)
            If (LCase$(nameofgivenID) = LCase$(monsterName)) Then
                lngCount = lngCount + 1
            End If
  ...
  CountOnFloorWiceps = lngCount
  Exit Function
goterr:
  CountOnFloorWiceps = 0
End Function
Public Function CountOnFloorWiceps2(idConnection As Integer, monsterName As String, XY As String) As Long
  'Dim aRes As Long
  'Dim myBpos As Long
  Dim myID As Long
  'Dim b As Byte
...
'goterr:
  ...
  Dim tileID As Long
  Dim nameofgivenID As String
  lngFloor = CLng(Trim$(XY))
  If (lngFloor > 5) Or (lngFloor < 1) Then
    lngFloor = 1
  End If
  z = myZ(idConnection)
  lngCount = 0
  For X = -lngFloor To lngFloor
    For y = -lngFloor To lngFloor
        For s = 1 To 10
          tileID = GetTheLong(Matrix(y, X, z, idConnection).s(s).t1, Matrix(y, X, z, idConnection).s(s).t2)
          If tileID = 97 Then
           nameofgivenID = GetNameFromID(idConnection, Matrix(y, X, z, idConnection).s(s).dblID)
  ...
  CountOnFloorWiceps2 = lngCount
  Exit Function
goterr:
  CountOnFloorWiceps2 = 0
End Function
...
Public Function getBlackdINI(ByRef par1 As String, ByRef par2 As String, _
 ByRef par3 As String, ByRef par4 As String, ByRef par5 As Long, ByVal par6 As String) As Long
